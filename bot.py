# bot.py — korrigierte Version: sync erst nach Command-Definitionen
...
def load_data() -> Dict[str, Any]:
    ...
    data.setdefault("events", []); data.setdefault("next_id", 1)
    return data
...
def new_id(data: Dict[str, Any]) -> int:
    nid = int(data.get("next_id", 1)); data["next_id"] = nid + 1; return nid
...
def next_occurrence(dt: datetime, recurrence: str) -> datetime:
    ...
    if recurrence == "weekly": return dt + timedelta(weeks=1)
    if recurrence == "monthly": return add_month(dt)
    return dt
...
intents.guilds = True
intents.members = True  # für Member-Auswahl in /ptermin
bot = commands.Bot(command_prefix="!", intents=intents)  # prefix ist inert, wir nutzen Slash
...
# ============= Slash commands definitions (all) ============
# (definitions must be present before we call sync)
...
async def termin_cmd(interaction: discord.Interaction, datum: str, uhrzeit: str, titel: str, erinnerung: str="30", wiederholung: str="none"):
    ...
    try: dt = parse_date_time(datum, uhrzeit)
    except Exception:
        await interaction.followup.send("❌ Datum/Uhrzeit ungültig. Beispiel: 08.02.2026 und 12:00", ephemeral=True); return
    reminders = parse_reminders(erinnerung)
    data = load_data(); eid = new_id(data)
    data["events"].append({"id": eid, "title": titel, "datetime": dt_to_iso(dt), "reminders": reminders,
                          "sent": [], "recurrence": wiederholung, "cancelled": False,
...
@app_commands.choices(wiederholung=CHOICES_REC)
async def ptermin_cmd(interaction: discord.Interaction, datum: str, uhrzeit: str, titel: str, erinnerung: str="30", wiederholung: str="none",
    ...
    try: dt = parse_date_time(datum, uhrzeit)
        ...
        await interaction.followup.send("❌ Datum/Uhrzeit ungültig.", ephemeral=True); return
    reminders = parse_reminders(erinnerung)
...
async def termine_cmd(interaction: discord.Interaction):
    ...
    if not events: await interaction.followup.send("📭 Keine aktiven Termine.", ephemeral=True); return
    lines = []
    for e in events[:25]:
        dt = dt_from_iso(e["datetime"]); rems = ",".join(str(m) for m in e.get("reminders", [])) or "—"
...
async def termine_all_cmd(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    data = load_data(); events = sorted(data["events"], key=lambda e: dt_from_iso(e["datetime"]))
    if not events: await interaction.followup.send("📭 Keine Termine gespeichert.", ephemeral=True); return
    lines = []
    for e in events[:25]:
        dt = dt_from_iso(e["datetime"]); rems = ",".join(str(m) for m in e.get("reminders", [])) or "—"
...
async def termin_absagen_cmd(interaction: discord.Interaction, termin_id: int):
    ...
    for e in data["events"]:
        if int(e.get("id",-1)) == int(termin_id) and not e.get("cancelled", False):
            e["cancelled"] = True; save_data(data)
            await interaction.followup.send(f"❌ Termin **{termin_id}** abgesagt.", ephemeral=True); return
...
@app_commands.describe(termin_id="ID aus /termine", datum="Optional: DD.MM.YYYY", uhrzeit="Optional: HH:MM",
                       titel="Optional: neuer Titel", erinnerung="Optional: z.B. 120,30,10",
                       wiederholung="Optional: none/daily/weekly/monthly")
@app_commands.choices(wiederholung=CHOICES_REC)
async def termin_edit_cmd(interaction: discord.Interaction, termin_id: int, datum: Optional[str]=None,
                          uhrzeit: Optional[str]=None, titel: Optional[str]=None, erinnerung: Optional[str]=None,
    ...
    if ev is None: await interaction.followup.send("❌ Termin-ID nicht gefunden.", ephemeral=True); return
    ...
    if datum is not None or uhrzeit is not None:
        cur = dt_from_iso(ev["datetime"]); d = datum if datum is not None else cur.strftime("%d.%m.%Y"); t = uhrzeit if uhrzeit is not None else cur.strftime("%H:%M")
        try: new_dt = parse_date_time(d, t)
        except Exception: await interaction.followup.send("❌ Neues Datum/Uhrzeit ungültig.", ephemeral=True); return
        ev["datetime"] = dt_to_iso(new_dt); ev["sent"] = []
    save_data(data); await interaction.followup.send(f"✅ Termin **{termin_id}** aktualisiert.", ephemeral=True)

# ============= startup: sync AFTER command definitions ============
@bot.event
async def on_ready():
    print(f"✅ Bot online als {bot.user}", flush=True)
    # Sync commands to the specified guild (done once)
    guild = discord.Object(id=GUILD_ID)
    if CLEAN_GLOBAL_COMMANDS:
        print("🧹 CLEAN_GLOBAL_COMMANDS active -> removing global commands ...", flush=True)
        bot.tree.clear_commands(guild=None)
        await bot.tree.sync()  # will clear global remote commands
        print("✅ Cleared global remote commands", flush=True)
